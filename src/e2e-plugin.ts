...
export class E2ePlugin extends GahPlugin {
  ...
  public async onInstall(existingCfg?: E2eConfig): Promise<GahPluginConfig> {
    ...
    const isHost =
      (await this.configurationService.getGahModuleType()) ===
      GahModuleType.HOST;
      ...
      this.registerEventListener("AFTER_GENERATE_SYMLINKS", async (event) => {
        ...
        if (event.module?.isHost) {
          ...
          await this.copyConfigFiles(
            event.module,
            "playwright.projects.config.json"
          );
          await this.generatePlaywrightProjectsConfig(
            event.module,
            allDepModules
          );
  ...
  private allFilterdRecursiveDependencies(
    ...
    const filterdModules = allModules.filter((mod) =>
      this.getPluginCfgFromModule(mod)
    );
  ...
  private async linkTestFiles(
    ...
    for (const depMod of allDepModules) {
      ...
      if (plugConf?.testDirectoryPath) {
        const sourceTestDirectoryPath = this.fileSystemService.join(
          depMod.basePath,
          plugConf.testDirectoryPath
        );

        if (
          await this.fileSystemService.directoryExists(sourceTestDirectoryPath)
        ) {
          await this.fileSystemService.ensureDirectory(distTestFolder);
          try {
            await this.fileSystemService.createDirLink(
              `${distTestFolder}/${depMod.moduleName!}`,
              sourceTestDirectoryPath
            );
          } catch (error) {
  ...
  private async linkSharedTestFolder(
    ...
    for (const depMod of allDepModules) {
      ...
      if (plugConf?.sharedHelperPath) {
        const plugDirectoryPath = this.extractDirectoryFromPluginPath(
        ...
        const sharedHelperSourcePath = this.fileSystemService.join(
          depMod.basePath,
          plugDirectoryPath[0],
          plugDirectoryPath[1]
        );
        if (
          await this.fileSystemService.directoryExists(sharedHelperSourcePath)
        ) {
  ...
  private async generatePlaywrightProjectsConfig(
    ...
    for (const depMod of allDepModules) {
      const plugConf = this.getPluginCfgFromModule(depMod);
      if (plugConf?.testDirectoryPath) {
        await this.addProjectToPlaywrightConfig(
          module.basePath,
          depMod.moduleName as string
        );
      }
    }
  }
  ...
  private async addProjectToPlaywrightConfig(path: string, moduleName: string) {
    ...
    const pwConfig: Array<any> = await this.fileSystemService.parseFile<
      Array<any>
    >(playwrightConfigSourcePath);
    ...
    await this.fileSystemService.saveObjectToFile(
      playwrightConfigSourcePath,
      pwConfig
    );
  }
  ...
  private async createTsconfigForTestModule(module: GahModuleData) {
    ...
    if (plugConf && plugConf.testDirectoryPath) {
      ...
      const tsconfigJsonSourcePath = this.fileSystemService.join(
        plugConf.testDirectoryPath,
        "tsconfig.json"
      );
      ...
      await this.fileSystemService.rename(
  ...
  private async addPathsToTsconfig(
    ...
    if (module.isHost) {
      tsConfigPath = this.fileSystemService.join(
        module.basePath,
        "tsconfig.spec.json"
      );
    } else {
    ...
    if (tsConfigPath) {
      const tsConfig = await this.fileSystemService.parseFile<TsConfig>(
        tsConfigPath
      );
      ...
      allDepModules.forEach((depMod) => {
        ...
        if (plugConf?.sharedHelperPath) {
          ...
          tsConfig.compilerOptions.paths[
            `@${depMod.packageName}/${depMod.moduleName}/test`
          ] = [
